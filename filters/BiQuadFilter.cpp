...
bool BiQuadFilter::processSingle(float** output, float** input, unsigned frameCount)
{
#if !defined(_M_ARM64)
    unsigned old_mxcsr = _mm_getcsr();
    _mm_setcsr(old_mxcsr | 0x8040);
    ...
    unsigned num_avx512_chunks = ((unsigned)channelCount - processedChannels) / avx512_width;
    if (num_avx512_chunks > 0) {
        processSingle_avx512(output, input, frameCount, processedChannels, num_avx512_chunks * avx512_width);
        processedChannels += num_avx512_chunks * avx512_width;
    }
#endif
    ...
    unsigned num_avx256_chunks = ((unsigned)channelCount - processedChannels) / avx256_width;
    if (num_avx256_chunks > 0) {
        processSingle_avx256(output, input, frameCount, processedChannels, num_avx256_chunks * avx256_width);
        processedChannels += num_avx256_chunks * avx256_width;
    }
#endif
#if !defined(_M_ARM64)
    ...
    unsigned num_sse128_chunks = ((unsigned)channelCount - processedChannels) / sse128_width;
    if (num_sse128_chunks > 0) {
        processSingle_sse128(output, input, frameCount, processedChannels, num_sse128_chunks * sse128_width);
        processedChannels += num_sse128_chunks * sse128_width;
    }
#endif

    if (processedChannels < (unsigned)channelCount) {
        processSingle_scalar(output, input, frameCount, processedChannels);
    }
...
#if defined(__AVX512F__) && !defined(_M_ARM64)
void BiQuadFilter::processSingle_avx512(float** output, float** input, unsigned frameCount, unsigned startChannel, unsigned numChannels)
{
    const unsigned simd_width = 8;
    for (unsigned i = startChannel; i < startChannel + numChannels; i += simd_width)
    {
        ...
        for (unsigned j = 0; j < frameCount; ++j)
        {
            __m256 _sample_f = _mm256_set_ps(input[i + 7][j], input[i + 6][j], input[i + 5][j], input[i + 4][j],
                input[i + 3][j], input[i + 2][j], input[i + 1][j], input[i + 0][j]);
            __m512d _sample = _mm512_cvtps_pd(_sample_f);
            ...
            __m256 result_f = _mm512_cvtpd_ps(result);
            float result_array[8];
            _mm256_storeu_ps(result_array, result_f);
            for (int k = 0; k < 8; ++k) output[i + k][j] = result_array[k];
        }
...
#if defined(__AVX2__) && !defined(_M_ARM64)
void BiQuadFilter::processSingle_avx256(float** output, float** input, unsigned frameCount, unsigned startChannel, unsigned numChannels)
{
    const unsigned simd_width = 4;
    for (unsigned i = startChannel; i < startChannel + numChannels; i += simd_width)
    {
        ...
        for (unsigned j = 0; j < frameCount; ++j)
        {
            __m128 _sample_f = _mm_set_ps(input[i + 3][j], input[i + 2][j], input[i + 1][j], input[i + 0][j]);
            __m256d _sample = _mm256_cvtps_pd(_sample_f);
            ...
            __m128 result_f = _mm256_cvtpd_ps(result);
            float result_array[4];
            _mm_storeu_ps(result_array, result_f);
            output[i + 0][j] = result_array[0];
            output[i + 1][j] = result_array[1];
            output[i + 2][j] = result_array[2];
            output[i + 3][j] = result_array[3];
...
#if !defined(_M_ARM64)
void BiQuadFilter::processSingle_sse128(float** output, float** input, unsigned frameCount, unsigned startChannel, unsigned numChannels)
{
    const unsigned simd_width = 2;
    for (unsigned i = startChannel; i < startChannel + numChannels; i += simd_width)
    {
        ...
        for (unsigned j = 0; j < frameCount; ++j)
        {
            __m128 _sample_f = _mm_set_ps(0.0f, 0.0f, input[i + 1][j], input[i + 0][j]);
            __m128d _sample = _mm_cvtps_pd(_sample_f);
            ...
            __m128 result_f = _mm_cvtpd_ps(result);
            float result_array[4];
            _mm_storeu_ps(result_array, result_f);
            output[i + 0][j] = result_array[0];
            output[i + 1][j] = result_array[1];
...
void BiQuadFilter::processSingle_scalar(float** output, float** input, unsigned frameCount, unsigned startChannel)
{
    for (unsigned i = startChannel; i < channelCount; ++i)
    {
        double cur_x1 = x1[i], cur_x2 = x2[i];
        ...
        for (unsigned j = 0; j < frameCount; ++j)
        {
            const double sample = static_cast<double>(inputChannel[j]);
            const double result = c_a0 * sample + c_b1 * cur_x1 + c_b2 * cur_x2 - c_a1 * cur_y1 - c_a2 * cur_y2;
            cur_x2 = cur_x1;
            cur_x1 = sample;
            cur_y2 = cur_y1;
            cur_y1 = result;
            outputChannel[j] = static_cast<float>(result);
        }
        x1[i] = cur_x1; x2[i] = cur_x2;
        y1[i] = cur_y1; y2[i] = cur_y2;
    }
