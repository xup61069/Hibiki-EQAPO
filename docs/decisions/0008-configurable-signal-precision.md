...
含 `Include` 或 `If` 等條件作用域的檔案，編輯器不從單一分頁猜測有效值，也不自動加入可能重複的宣告；標頭開關與選單均停用，並提示以設定文字編輯精度。標準的 `Device` 與 `Stage` 設定則可直接由開關修改。引擎仍支援這些設定的有效精度宣告。後續若擴充此控制，必須共用有效設定解析及來源定位，不能為更新 UI 狀態而載入外掛。
