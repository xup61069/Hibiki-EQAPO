...
「設定 → 雙精度」修改目前設定檔，遵守既有即時儲存、手動儲存與復原流程。沒有 precision 宣告時預設勾選 64 位元。

含 Include、Device、If、Stage 或動態運算等作用域的檔案，編輯器不從單一分頁猜測有效值，也不自動加入可能重複的宣告；選單停用並提示以設定文字編輯精度。引擎仍支援這些設定的有效精度宣告。後續若擴充此選單，必須共用有效設定解析及來源定位，不能為更新選單狀態而載入外掛。
